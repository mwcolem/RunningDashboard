...
- **`backend/app/main.py`** — FastAPI app with lifespan (attempts Garmin login on startup, logs warning and continues if it fails), CORS middleware (`GET`/`POST`), global exception handlers (`GarminConnectTooManyRequestsError` → 429, `GarminConnectAuthenticationError` → 401), and `POST /api/refresh` (evicts the TTL cache, returns `{"cleared": n}`).
- **`backend/app/config.py`** — `pydantic-settings` reads `.env` for `GARMIN_EMAIL`, `GARMIN_PASSWORD`, `CACHE_TTL_SECONDS`, `CORS_ORIGINS`.
- **`backend/app/garmin_client.py`** — Singleton Garmin client wrapper with in-memory TTL cache (`dict[str, tuple[float, Any]]`). All Garmin API calls go through here. Cache TTLs: activities list 600s, daily health metrics 300s, training metrics 600s, activity detail/splits 3600s (immutable), gear 3600s. Uses `cast()` throughout to satisfy mypy strict mode over the untyped garminconnect library. `clear_cache()` drops every entry and returns the count — data only, so the OAuth token store is untouched and no SSO re-login is triggered.
...
- **`backend/app/routers/stats.py`** — `/api/stats/mileage` (year/month/week totals in miles), `/api/stats/daily?start=&end=` (miles per day over a range; both dates are pattern-validated `YYYY-MM-DD`), `/api/stats/goals` (reads `goals.json` at project root, joins with mileage data to compute progress).
...
- **`frontend/src/api/client.ts`** — Fetch wrapper and all TanStack Query hooks with typed responses. All server state flows through here. `useRefreshAll()` is the one mutation: it POSTs `/api/refresh`, then invalidates every query on success.
...
- **`frontend/src/data/trainingPlan.ts`** — Static 24-week 50-mile ultramarathon plan (Relentless Forward Commotion / Hart Strength & Endurance Coaching), transcribed from the source sheet. Exports `PLAN` (weeks with dated days), `RACE_DATE`, `CELEBRATION`, `PEAK_MI`, `PLAN_START`/`PLAN_END`, and `weekFor(date)`. Each `PlanDay` carries a `targetMi` parsed from its cell text — see "Plan targets" below. The plan itself is local data; actual mileage is fetched separately by the Training page.
...
- **`frontend/src/components/RefreshButton.tsx`** — Dashboard header button wired to `useRefreshAll()`; spins its icon while pending and shows "Refresh failed" on error.
...
- **`frontend/src/pages/Training.tsx`** — 24-week plan calendar. Summary strip (current week, phase, weekly total, days to race), a hero card for the current week, and the full 24-week grid with per-week totals and cycle labels. The grid overlays actual mileage from `useDailyMileage(PLAN_START, min(today, PLAN_END))` — see "Plan targets" below. The summary strip and hero week stay plan-only.
...
- **Manual refresh**: the TTL cache means a plain frontend refetch re-serves stale data, so the refresh button clears the backend cache first (`POST /api/refresh`) and only then invalidates the TanStack Query cache. Every click costs a full round of live Garmin calls — it is deliberately manual, with no auto-refresh interval, to stay clear of the rate limit.
...
- **Plan targets and the hit marker**: `targetFor()` in `trainingPlan.ts` derives each day's `targetMi` from its cell text. Time-based cells (`60 minutes`, `2.5 hours`) are excluded first — they lead with a number that is not a distance — then the leading integer wins, so `10-12` resolves to **10** (a range is a floor, not a ceiling), `4 (speed)` to 4, and `31 (50K)` to 31. `REST` and `Active Recovery` yield null. A day whose `targetMi` is null still shows actual miles but is never marked hit or missed; a day with a target shows `—` when nothing was logged. Weekly totals compare actual against `week.totalMi` but are only ticked once the week is complete, since a mid-week total is always short.
