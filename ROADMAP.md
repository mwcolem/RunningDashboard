...
├── .github/
│   └── workflows/
│       └── ci.yml        # ruff/mypy/pytest (backend) + eslint/build (frontend)
...
10. ✅ **CI** — GitHub Actions: ruff + mypy + pytest (backend), eslint + `npm run build` (frontend). Tests mock Garmin client; no credentials needed in CI.
